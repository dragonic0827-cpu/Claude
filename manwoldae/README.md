...
| 이름표 · 유적 · 평면도 · 역사 · 도움말 | `L` · `R` · `M` · `I` · `H` | 도구 막대 |
...
- **화질** — 높음·보통·낮음(그림자 해상도, 숲 밀도, 해상도 배율). 계속 그리는 동안 33 fps 아래로 느리면 해상도를, 그래도 느리면
...
안내 여행 12곳과 걷기에서 잰 값입니다(한 프레임의 본 그리기 기준). 그림자 맵은 빛이나 보는 곳이 바뀔 때만 따로 다시 그립니다.

| 화질 | 그리기 호출 | 삼각형 | 그림자 맵을 다시 그리는 프레임에 더해지는 양 |
|---|---|---|---|
| 높음 (데스크톱, 1280×800) | 약 120–250회 | 약 55만–94만 개 (그중 숲 약 24만–42만) | 90–340회, 35만–72만 개 |
| 보통 (휴대폰 기본) | 약 130–200회 | 약 59만–69만 개 (숲 약 14만) | 90–170회, 25만–40만 개 |
| 낮음 (그림자 없음) | 약 170–200회 | 약 65만–76만 개 (숲 약 8만) | — |

전각은 거리에 따라 세 단계(높음·보통·낮음)로 바뀌고, 경계에서 깜박이지 않게 10 % 여유를 둡니다. 가장 자세한 단계는 처음부터 짓지 않고
카메라가 다가올 때 짓습니다(비행 중에는 60 ms 에 한 채씩). 그래서 불러오기가 빠르고 메모리를 덜 쓰는 대신, 순간 이동으로 가까이 가면
그 자리의 전각 몇 채를 한 번에 짓느라 잠깐 멈출 수 있습니다. 가장 먼 단계는 무늬 텍스처 대신 평균색을 칠한 두 메시(기단·몸체)이고,
그 기단과 서까래 마구리·금동 장식·편액은 그림자를 드리우지 않습니다. 계단·다리·담장은 재질별로, 회랑은 일곽(대지)별·재질별로 합쳐 그려 화면 밖 일곽은 건너뜁니다.
