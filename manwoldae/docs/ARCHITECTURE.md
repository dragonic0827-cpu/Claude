...
| | `createBuildingLOD(def, mats, opts)` → `LOD` | high·medium·low 세 단계(전환에 10 % 되돌림 여유). high 는 카메라가 medium 거리의 1.5배 안에 들어올 때 지음 — `lod.userData.ensureHigh()` 로 바로 지을 수도 있고, 새로 지은 단계는 폐허 모드·선택 강조 상태를 따름. low 는 평균색 정점색 두 메시(`foundation`·`superstructure`) |
...
- 그림자: 건물·계단·담장은 `castShadow = receiveShadow = true`, 지형은 `receiveShadow` 만. 예외: 전각의 아주 작은 부재(`rafterEnd`·`gilt`·`plaque`)와 low 단계 기단은 드리우지 않음.
...
- **전각 LOD 와 그림자**: `buildOne` 이 전각마다 `createBuildingLOD` 를 부릅니다. 그림자 설정은 `building.js` 가 메시마다 정하므로 main 에서 덮어쓰지 않습니다
  (서까래 마구리·금동 장식·편액과 먼 단계(low)의 기단은 `castShadow = false` — 창호·판문·포벽은 빛이 새지 않게 드리움).
  LOD 단계가 바뀌면 그리기 루프의 `lodSignature()` 가 달라져 그림자 맵을 다시 그립니다.
